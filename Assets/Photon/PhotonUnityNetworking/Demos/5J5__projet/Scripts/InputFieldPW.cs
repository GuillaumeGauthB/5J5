...
public class InputFieldPW : MonoBehaviour
{
    string passwordRoomPrefKey = "code_value"; // nom du joueur
    ...
    void Start()
    {
        ...
        if (_inputField != null)
        {
            ...
            if (PlayerPrefs.HasKey(passwordRoomPrefKey))
            {
                // l'utiliser
                defaultName = PlayerPrefs.GetString(passwordRoomPrefKey);
    ...
    public void OnPlayButtonPressed(string Value)
    {
        ...
        PlayerPrefs.SetString(passwordRoomPrefKey, Value);
    }
}